...
class Defaults(Enum):
    ...
    # EventBridge Tool Defaults # TODO: Move to an some tool submodule eventually
...
class Config:
    ...
    # DB TABLES
    SESSION_TABLE: str = field(
        default=os.environ.get(Defaults.SESSION_TABLE.name, Defaults.SESSION_TABLE.value)
    )
    LEAD_TABLE: str = field(
        default=os.environ.get(Defaults.LEAD_TABLE.name, Defaults.LEAD_TABLE.value)
    )
    AGENT_CONFIG_TABLE: str = field(
        default=os.environ.get(Defaults.AGENT_CONFIG_TABLE.name, Defaults.AGENT_CONFIG_TABLE.value)
    )

    SESSION_TABLE_FRIENDLY_NAME: str = field(
        default=os.environ.get(
            Defaults.SESSION_TABLE_FRIENDLY_NAME.name, Defaults.SESSION_TABLE_FRIENDLY_NAME.value
        )
    )
    LEAD_TABLE_FRIENDLY_NAME: str = field(
        default=os.environ.get(
            Defaults.LEAD_TABLE_FRIENDLY_NAME.name, Defaults.LEAD_TABLE_FRIENDLY_NAME.value
        )
    )
    ...
    SECRET_STORE: str = field(
        default=os.environ.get(Defaults.SECRET_STORE.name, Defaults.SECRET_STORE.value)
    )

    EVENTBUS_FRIENDLY_NAME: str = field(
        default=os.environ.get(
            Defaults.EVENTBUS_FRIENDLY_NAME.name, Defaults.EVENTBUS_FRIENDLY_NAME.value
        )
    )
    SECRET_STORE_FRIENDLY_NAME: str = field(
        default=os.environ.get(
            Defaults.SECRET_STORE_FRIENDLY_NAME.name, Defaults.SECRET_STORE_FRIENDLY_NAME.value
        )
    )
    ...
    # MODES
    RECOGNIZED_MODES: list[str] = field(
        default_factory=lambda: [x.value for x in Defaults if x.name.startswith("MODE_")]
    )

    def __post_init__(self):
        if self.MODE not in self.RECOGNIZED_MODES:
            raise ValueError(
                f"Environment variable MODE={self.MODE} must be one of {self.RECOGNIZED_MODES}"
            )
    ...
    def set_dynamic_values(self):
        _logger = get_logger()
        dynamic_attributes = [
            attrib.replace("_FRIENDLY_NAME", "")
            for attrib in self.__dict__
            if attrib.endswith("_FRIENDLY_NAME")
        ]
        ...
        for attrib in dynamic_attributes:
            ...
            _logger.debug(
                f"{attrib} not defined in environment variables, searching for friendly name {attrib_friendly_name}"
            )
        ...
        if no_friendly_name or undefined_resources:
            ...
            for attrib, attrib_friendly_name in undefined_resources:
                print(
                    f"ERROR: Can't find {attrib_friendly_name} values from your central infrastructure"
                )
