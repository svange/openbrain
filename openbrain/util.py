...
class Config:
    ...
    # MODES
    RECOGNIZED_MODES: list[str] = field(default_factory=lambda: [x.value for x in Defaults if x.name.startswith("MODE_")])

    def __post_init__(self):
        if self.MODE not in self.RECOGNIZED_MODES:
            raise ValueError(f"Environment variable MODE={self.MODE} must be one of {self.RECOGNIZED_MODES}")
